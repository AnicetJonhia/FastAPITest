...
def create_item(item_in: ChecklistCreate, db: Session = Depends(get_db), auth_user=Depends(require_role(["SUPERADMIN","RH","DEPT"]))):
...
def complete_item(item_id: int, db: Session = Depends(get_db), auth_user=Depends(get_current_user)):
    ...
    # optionally ensure auth_user is owner or admin
    if item.user_id != auth_user.id and getattr(auth_user, "role", None) not in ("SUPERADMIN","RH","MANAGER"):
